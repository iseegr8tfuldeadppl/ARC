...
def mouthChanged(x):
    global armPositions
    armPositions[viewed_armPosition]["mouth"] = x
    notifySave()
def bottomChanged(x):
    global armPositions
    armPositions[viewed_armPosition]["bottom"] = x
    notifySave()
def tiltChanged(x):
    global armPositions
    armPositions[viewed_armPosition]["tilt"] = x
    notifySave()
def spineChanged(x):
    global armPositions
    armPositions[viewed_armPosition]["spine"] = x
    notifySave()
def updateArmSliders():
    cv2.setTrackbarPos('Mouth', 'armSliders', armPositions[viewed_armPosition]["mouth"])
    cv2.setTrackbarPos('Bottom', 'armSliders', armPositions[viewed_armPosition]["bottom"])
    cv2.setTrackbarPos('Tilt', 'armSliders', armPositions[viewed_armPosition]["tilt"])
    cv2.setTrackbarPos('Spine', 'armSliders', armPositions[viewed_armPosition]["spine"])

def switchViewedPosition(event, x, y, flags, param):
    global button_down, viewed_armPosition, armPosition, armUpdated
...
# Arm: Functions
def wrap2(index):
    if index >= 4: #len(armPositions)
        return 0
    elif index < 0:
        return len(armPositions)-1
    else:
        return index
...
def checkArmUpdatedManually():
    ...
    if armUpdated:
        armUpdated = False
        go_to_coordinates(MSFromPercent(armPositions[armPosition]["mouth"], MOUTH_MIN, MOUTH_MAX), \
                            MSFromPercent(armPositions[armPosition]["bottom"], BOTTOM_MIN, BOTTOM_MAX), \
                            MSFromPercent(armPositions[armPosition]["tilt"], TILT_MIN, TILT_MAX), \
                            MSFromPercent(armPositions[armPosition]["spine"], SPINE_MIN, SPINE_MAX))
...
def robotArm():
    ...
    if time.time() - last_go_arm_execusion >= delay_between_arm_execusions:
        go_to_coordinates(MSFromPercent(armPositions[armPosition]["mouth"], MOUTH_MIN, MOUTH_MAX), \
                            ...
                            MSFromPercent(armPositions[armPosition]["spine"], SPINE_MIN, SPINE_MAX))
        armPosition += 1
        return armPosition < len(armPositions)
